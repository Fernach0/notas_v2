...
export default function ActividadForm({ idParcial, item, onSubmit, isLoading }: Props) {
  const { register, handleSubmit, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: item
      ...
      : { tipoActividad: 'TAREA', valorMaximo: 100 },
  });
    ...
    <form onSubmit={handleSubmit((d) => onSubmit({ ...d, idParcial }))} className="space-y-4">
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <select {...register('tipoActividad')} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
        ...
        <div>
          ...
          <input type="number" {...register('valorMaximo')} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          {errors.valorMaximo && <p className="mt-1 text-xs text-red-500">{errors.valorMaximo.message}</p>}
        </div>
      </div>

      <div>
        <label className="block text-sm font-medium text-slate-700 mb-1">Título</label>
        <input {...register('tituloActividad')} placeholder="Ej: Tarea 1" maxLength={20} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
      </div>

      <div>
        <label className="block text-sm font-medium text-slate-700 mb-1">Descripción</label>
        <textarea {...register('descripcion')} rows={3} maxLength={200} placeholder="Instrucciones para los estudiantes..." className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 resize-none" />
      </div>

      <div className="grid grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium text-slate-700 mb-1">Inicio entrega</label>
          <input type="date" {...register('fechaInicioEntrega')} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
        ...
        <div>
          ...
          <input type="date" {...register('fechaFinEntrega')} className="w-full rounded-lg border border-slate-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
